...
const SessionContent = props => {
  const {
    ...
    activeSessionIndex: activeStep,
    selectSessionIndex
  } = props

  if (isMasterSessionLoading) return <PaneProgress />

  const steps = getAllMasterSessions
  const maxSteps = steps.length

  const handleUp = () => {
    selectSessionIndex(activeStep - 1 < 0 ? 0 : activeStep - 1)
  }
  const handleNext = () => {
    selectSessionIndex(activeStep + 1 > maxSteps ? maxSteps : activeStep + 1)
  }
  const handleStepChange = activeStep => {
    selectSessionIndex(activeStep)
  }
...
export default withStyles(styles)(SessionContent)
